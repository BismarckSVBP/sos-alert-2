...
const viewerSockets = {};
const senderSockets = {};
const latestLocation = {}; // Stores last known location for each session
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-room", ({ sessionId, role }) => {
      ...
      if (role === "sender") {
        ...
        if (!viewerSockets[sessionId]) viewerSockets[sessionId] = new Set();
        viewerSockets[sessionId].add(socket.id);
        console.log(`👁️ Viewer joined session ${sessionId}: ${socket.id}`);
        const senderId = senderSockets[sessionId];
        if (senderId) {
    ...
    socket.on("end-call", ({ sessionId }) => {
      ...
      delete senderSockets[sessionId];
      delete viewerSockets[sessionId];
      delete latestLocation[sessionId];
      console.log(`📴 Stream ended for session ${sessionId}`);
    });
    ...
    // Late joiner triggers this:
    socket.on("new-tracker-viewer", ({ sessionId, viewerId }) => {
      const loc = latestLocation[sessionId];
      if (loc && viewerId) {
        io.to(viewerId).emit("receive-location", loc);
        console.log(`📡 Sent last location to new viewer ${viewerId}`);
      }
    });

    // Send location (to all or specific viewer)
    socket.on("send-location", async ({ sessionId, latitude, longitude, timestamp, targetId }) => {
      try {
        const location = await Location.create({
          sessionId,
          latitude,
        ...
        latestLocation[sessionId] = locationData;

        if (targetId) {
          io.to(targetId).emit("receive-location", locationData);
        } else {
          io.to(sessionId).emit("receive-location", locationData);
        }
      } catch (err) {
        console.error("📍 Location saving error:", err.message);
      }
    ...
    // For completeness: Pause/resume events (not main focus in viewer logic)
    socket.on("stop-location", ({ sessionId }) => {
      io.to(sessionId).emit("location-paused");
      console.log(`⏸️ Location paused for session ${sessionId}`);
    });
    ...
    // 🚩 CRUCIAL: terminate-location --> ALL TRACKERS GET "terminate-location", then page will redirect
    socket.on("terminate-location", ({ sessionId }) => {
      io.to(sessionId).emit("terminate-location"); // 👈 THIS IS WHAT YOUR CLIENT TRACKER CODE LISTENS FOR
      socket.leave(sessionId);
      delete latestLocation[sessionId];
      console.log(`🛑 Location terminated for session ${sessionId}`);
    });
